...
angular.module("myapp")
    .controller("detailCtrl", function ($scope, $http, $stateParams, shopCart,$ionicHistory) {
        $scope.back=function(){
            $ionicHistory.goBack()
        };

        $('a.navBtn').on('click',function(){
            $(this).addClass('ac').siblings().removeClass('ac');
            var index=$(this).index()-1;
            $(this).parents('ion-header-bar').siblings().find('.detailCon').eq(index).show().siblings().hide();
        })
